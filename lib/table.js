...
prop.exec = function (path, method, roles) {
  var pathEntries = this.pathEntries
  var roleIndexes = this.roleIndexes
  var roleEntries = this.roleEntries
  ...
  for (i = 0; i < l; i++) {
    var index = matchEntries[i]
    var pathEntry = pathEntries[i]
    if (pathEntry.exec(path, method)) {
      return [index, pathEntry]
    }
  }
  return null
}

prop.areAnyAllow = function (path, methods, roles) {
  var result = this.exec(path, methods, roles)
  if (result && result[1].action === 'allow') {
    return true
  }
  return false
}
